...
# "Unknown" is not a real organization - it means GTD analysts couldn't attribute
# the attack to anyone. Leaving it in makes the model learn to predict "we don't
# know", which is not a useful or honest target class, and since Unknown is often
# 40-50%+ of all rows it would dominate accuracy while being a meaningless "win".
# Set to False if you want to keep it in anyway.
...
def stage_tune():
    ...
    # Range validated empirically on the real GTD data: min_samples_leaf controls
    # how large each tree grows (and therefore the saved model's file size) far
    # more effectively than max_depth alone, with 89-way-classification-sized
    # forests otherwise reaching multiple GB on disk.
    param_grid = {
